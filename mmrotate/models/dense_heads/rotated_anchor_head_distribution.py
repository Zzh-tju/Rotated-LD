...
class RotatedAnchorHeadDistribution(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(RotatedAnchorHeadDistribution, self).__init__()
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, anchors in zip(cls_score_list,
            ...
            encode_pred = self.integral(bbox_pred[:,:((self.reg_max + 1) * 4)].reshape(-1, self.reg_max + 1))
            encode_angle_pred = self.integral_angle(bbox_pred[:,((self.reg_max + 1) * 4):].reshape(-1, self.reg_max + 1))
            bbox_pred = torch.stack([encode_pred[:,0], encode_pred[:,1], encode_pred[:,2], encode_pred[:,3], encode_angle_pred], dim=1)
